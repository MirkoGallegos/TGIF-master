               ...
               pro.innerHTML=tabla
                    ...
                    $(document).ready(function () {
                        $('#senate-data').DataTable({
                            //Estos parametros deshabilitan las características de paginación, ordenamiento e info en datatables
                            "paging": false,
                            "ordering": false,
                            //Sobreescribo el dom para ocultar la barra de busqueda de datatables sin deshabilitar la función de búsqueda, porque sino,
                            //no funciona el filtro. Además acomodo la información en el pie de la tabla poniendo la t de 'tabla' y después la i de 'información'
                            "dom": 'ti',
                            initComplete: function () {
                                //Esta función genera el filtro para la columna 'state'
                                this.api().columns(2).every(function () {
                                    var column = this;
                                    var select = $('<select><option value="">Show all</option></select>')
                                         //Agrega el filtro a un span fuera de la tabla
                                        .appendTo( $('#stateSearch').empty().text('Filter by State: ') )
                                        //Agrega el filtro a la cabecera de la columna
                    ...
                    $(document).ready(function () {
                        var table = $('#senate-data').DataTable();
                        $('input:checkbox').on('change', function () {
                            //build a regex filter string with an or(|) condition
                            var parties = $('input:checkbox[name="partyAffiliation"]:checked').map(function () {
                                return '^' + this.value + '$';
                            }).get().join('|');
                            //filter in column 1, with an regex, no smart filtering, not case sensitive
                            table.column(1).search(parties, true, false, false).draw(false);
                        })
